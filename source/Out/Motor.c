...
void PIDControl(pid_control_config_t* base)
{
  ...
  base->IncrementSpeed = (int16_t)((base->PidCore.Ki) * (base->PidCore.state[NowError]))+
                         (int16_t)(base->PidCore.Kp * (base->PidCore.state[NowError] - base->PidCore.state[lastError]));
  
  base->IncrementSpeed+= (int16_t)(base->PidCore.Kd)*(base->PidCore.state[NowError] - 2*base->PidCore.state[lastError]+base->PidCore.state[lastestError]);
