...
void GetADCVal(uint16_t* vals)
{
    ...
    for(uint8_t i = 0; i < 5; i++)
    {
        ...
        while (kHSADC_ConverterAEndOfScanFlag != (kHSADC_ConverterAEndOfScanFlag & HSADC_GetStatusFlags(HSADC0)));
        temp[1] += HSADC_GetSampleResultValue(HSADC0, 0U);
        HSADC_ClearStatusFlags(HSADC0, kHSADC_ConverterAEndOfScanFlag);
        
        HSADC_DoSoftwareTriggerConverter(HSADC0, kHSADC_ConverterA);
        while (kHSADC_ConverterAEndOfScanFlag != (kHSADC_ConverterAEndOfScanFlag & HSADC_GetStatusFlags(HSADC0)));
        temp[0] += HSADC_GetSampleResultValue(HSADC0, 1U);
        HSADC_ClearStatusFlags(HSADC0, kHSADC_ConverterAEndOfScanFlag);
    }
